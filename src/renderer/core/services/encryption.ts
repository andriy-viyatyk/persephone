...
export async function encryptText(
    ...
    try {
        ...
        const result =
            ENCRYPTION_VERSION_V1 + arrayBufferToBase64(combined.buffer);
        return result;
    } catch (error) {
...
async function decryptTextV1(
    ...
    const dataWithoutVersion = encryptedText.substring(
        ENCRYPTION_VERSION_V1.length
    );

    // Decode the combined data
    const combined = Uint8Array.from(atob(dataWithoutVersion), (c) =>
        c.charCodeAt(0)
    );
