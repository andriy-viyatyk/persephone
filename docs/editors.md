...
- **Request body types** — choose between **none**, **x-www-form-urlencoded** (key-value editor), or **raw** body with a language sub-selector (JSON, JavaScript, HTML, XML, plaintext). Raw body uses Monaco Editor with syntax highlighting. Content-Type header is set automatically when you switch body type or language.
