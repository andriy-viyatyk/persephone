...
| 2.0b | [US-353](../tasks/US-353-category-panel-tree-provider/README.md) | Replace CategoryTree with TreeProviderView in LinkCategoryPanel | Replace custom `CategoryTree` with `TreeProviderView` + `LinkTreeProvider`. Add `getLabel` and `rootLabel` props to TreeProviderView. Two modes: `showLinks={false}` (categories only, when main editor) and `showLinks={true}` (categories with links, when secondary-only). Drag-drop works natively via unified `LinkDragEvent`. | Planned |
