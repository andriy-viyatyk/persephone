...
- [x] US-1483: Bundled board registry and discovery
- [x] US-1484: Stable identity for bundled boards across install paths
- [x] US-1485: Built-in tab presentation and the Disable action
- [x] US-1486: The board's prebuilt `lib/`, generated once and committed
- [x] US-1487: The Excalidraw board
- [x] US-1488: Capability routing into the board, and the payload measurement
- [x] US-1489: Board navigation return URLs
- [x] US-1490: The Excalidraw board's library flow

Excalidraw now ships as a *bundled board* — a board inside the installer, registered by provenance
with no trust dialog and no `trustedBoards.txt` write, presented under Tools & Editors → Built-in
with a Disable action, and never under Registered boards. It opens every `.excalidraw` file at
priority 60 and handles `image.edit` / `diagram.edit` for the image viewer, SVG, Mermaid and the
snip tool. `editors/draw` deliberately remains behind it as a working fallback; deleting it is
EPIC-110.

Two roadmap corrections were found while verifying and became decisions: Phase F.2's "first run
copies the board into the install dir" is unworkable because the install directory is not writable
without elevation (D2), and Phase F.3's `editorPriority: 50` would have lost to the very editor the
board replaces, since built-ins win exact ties (D7).

The board's `lib/` is generated by hand and committed (D9) rather than rebuilt by any Persephone
build — it is a pure function of the pinned Excalidraw version, so rebuilding it per `npm run dist`
would buy nothing and make every build depend on a bundler run that can fail. Measurement during
US-1486 corrected the size estimate: the board needs **33** bare specifiers, not the 15 visible in
`index.js`, and one of them (`@excalidraw/mermaid-to-excalidraw`) is 3.4 MB on its own. It was kept
deliberately — dropping it would ship a regression against the editor being replaced — bringing the
committed tree to 7.3 MB, with Xiaolai (13 MB of Chinese handwriting) and 54 locales excluded.

Three things are worth carrying into EPIC-110. **D3 did not survive implementation**: it predicted
the board would take over `draw-view`'s row so the user saw one "Drawing" entry, but the board
contributes its own row labelled "Excalidraw" while the built-in keeps its pinned slot — two
entries, which is honest for a coexistence epic but means EPIC-110 must treat inheriting the pin and
label position as explicit work. **The `DataHandle` store stays deferred on measured evidence**:
the worst p95 structured-clone cost was 6.1 ms against EPIC-108 D7's ~50 ms trigger, and no
ordinary-use payload crossed the 8 MiB cap. And **a board whose third-party site returns in the same
tab is still not heard** — a minted `.invalid` return URL cannot resolve, so the navigation dies at
DNS before `did-navigate` fires; the `window.open` path is what works, and fixing the other needs
`did-fail-load` relayed from `browser-service.ts`.

`/review` at close found the one real defect: **Disable was a one-way door.** The action shipped
without an Enable counterpart, and since disabling removes the row, the only way back was
hand-editing `settings.json`. Fixed by giving a disabled bundled board an inert, dimmed row that
carries only Enable — without weakening D4, which still gates the creatable item.
