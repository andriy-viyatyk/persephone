...
Trusted boards may extend editor switching with manifest `fileMasks` and switch-only
`contentMasks`; content masks can identify an untitled or otherwise unmatched page without
changing which editor wins file-open resolution. A trusted board that declares a safe `guides`
folder contributes an isolated guide mount under `installed-boards/<board-id>/`, shared by the
About browser, `F1`, MCP guide tree, and guide search.
