...
Trusted boards can join this host-less folder family independently of their file association. A
...
switch list keeps built-ins first and appends matching trusted boards in trust order. A resolved
...
folder resolver and trusted `folderEditorMasks`, but `getFolderEditorsForFolder(folderPath)` keeps
the switch order independent of priority: built-in candidates first, then trusted folder boards.
The registry is intentionally inert until a board is trusted; a manifest that only declares the
new folder axis does not affect ordinary folder opening until that trust projection exists.
