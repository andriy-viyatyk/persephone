...
| `board-view` | `BoardEditorModel` | folders carrying `board-manifest.json` (opened via `persephone-board://`; also acts as a custom editor for files via `fileMasks` or for folders via `folderEditorMasks` — see "Custom-Editor Boards") | — | — |
...
- **Non-text editors** override `onGetMenuItems()` to contribute their own items (Git Tree → "Open Git Root Folder" / "Copy Remote URL"; Image/Archive → the file-path items; Board → "Copy Board Path" / "Open Board Folder"). The Board editor appends its items after the base result so a content-host board retains its inherited text-file menu.
...
The Image editor's script facade also uses `rasterToPngBlobWithDimensions(src, maxDimension)` for
its read-only inline MCP result. That helper preserves the source aspect ratio, limits the longer
side without changing the natural-size `IImageExport.exportPng()` contract, and reports both applied
and original dimensions alongside the PNG blob. It remains view-independent, so the facade can read
an inactive image page without using viewport geometry.
...
Trusted or bundled boards can join this host-less folder family independently of their file association. A
board declares `folderEditorMasks` to match the folder itself and optional `folderEditorPriority`
to compete for the default folder editor; these fields are a separate manifest axis from
`folderMasks`, which only narrows file associations. The merged folder resolver compares trusted
board claims with the built-in resolver (strictly-greater priority; built-ins win ties), while the
switch list keeps built-ins first and appends matching board candidates in registry order. A resolved
board uses the generic `folder-editor://` link, which carries `{ editorId, anchorFolder }` as
UTF-8-safe base64 JSON; its lifecycle path validates the current trusted claim before constructing
the board.
...
A trusted or bundled **Board** can register itself as the editor for a file type, so it appears in the editor switch next to Monaco (and can become the default open target) — the same extensibility the built-in editors have, but authored entirely outside Persephone's static editor table. A board declares the association in its `board-manifest.json`:
...
- `folderEditorMasks` — optional folder globs that match the **folder itself**, creating a
...
These fields are honored only for trusted boards and enabled bundled boards. Bundled boards are
discovered from `assets/boards/`, are not added to the trust file, and are presented in the
Built-in editor list; disabling one removes its editor and other manifest registrations live.

**The mask predicates.** `matchesBoardMasks(filePathOrName, fileMasks, folderMasks)` is the single
entry point for the file axis — every consumer (the registry and published-catalog matchers) goes
through it rather than testing masks itself. A board claims a file iff the **basename** matches one
of `fileMasks` and either there are no folder masks or the file's **parent folder** matches one of
them. Folder masks only narrow: a board with no `fileMasks` has no file association even when
`folderMasks` is present. The separate `matchesFolderEditorMasks(folderPath, folderEditorMasks)`
predicate matches the absolute folder itself; a folder-only association is valid.
...
**Two registries, merged.** Board editors are runtime-discovered, so they are **not** injected into the static `editorRegistry`. They live in a separate reactive registry, `customEditorRegistry` (`editors/board/custom-editor-registry.ts`), which enumerates trusted roots plus the app-owned bundled-board registry, reads each manifest, and maps file → claiming boards. It reacts to trust, bundled-source, and disabled-board changes (a removed or disabled association drops live) and re-reads manifests on refresh — there is no filesystem watcher, mirroring the Agent Tools `registeredTools` precedent. Each associated board is a distinct **virtual editor id** `board-editor:<boardRoot>` (`boardEditorId` / `parseBoardEditorId`), where the remainder after the prefix is the board root verbatim (original case; may contain `:` and `\` — parse by prefix, never by split).
...
the switch order independent of priority: built-in candidates first, then board candidates.
The registry is intentionally inert until a board is trusted or shipped as enabled bundled content;
a manifest that only declares the new folder axis does not affect ordinary folder opening otherwise.

**Construction & switch.** When resolution yields a `board-editor:<root>` id, `PagesLifecycleModel.buildEditorById` decodes the root (in a branch placed *before* the text-fallback, so an unrecognized board id can't silently open as text) and builds a `BoardEditorModel` initialized with the target file path. For a folder target, `createEditorFromFolder(editorId, folderPath)` validates that the virtual id is still a trusted registry entry claiming that folder, then calls the board's folder factory with the board root and claimed folder as independent values. `PageModel.switchMainEditor` branches on the `board-editor:` prefix *before* the `editorRegistry.getById` lookup (which throws on unregistered ids): it runs the old editor's `confirmRelease()` guard (abort on cancel — reusing navigation's unsaved-changes prompt), then rebuilds through `createEditorFromFile` or the folder path as appropriate (a board has no shared content host to transfer). The switch widget (`SwitchWidgetView` in `PageToolbarView.ts`, and its direct `mountVanilla` use in `BoardToolbar.ts`) merges board options from the file- or folder-keyed registry projection and resolves labels from the registry rather than `editorRegistry.getById`. Folder switches preserve the built-in-first order and keep a currently open board recoverable while trust refresh removes it from newly selectable candidates. Text editors inherit the file behavior; a **no-host** built-in that can be a simple board's peer overrides it explicitly — the Archive editor does (exposing its `archiveUrl` as `filePath`, returning `["archive-view"]`), so it can be the built-in peer of a board claiming a ZIP-based file such as `.xlsx` / `.docx`.
...
**Identity & persistence.** A `BoardEditorModel` acting as a custom editor reports its `editorId` as the dynamic `board-editor:<root>` so the switch widget matches; `getRestoreData()` pins the persisted id to `"board-view"` and stores `boardRoot` beside the independent `filePath` or `folderPath`. Restore revalidates a folder claim against the current trusted registry before constructing it, and an invalid claim falls back to Folder View rather than inferring the folder from the installed board root. MCP/automation board detection uses `isBoardEditorId` (true for both `"board-view"` and any `board-editor:<root>`); match such a page by its `boardRoot` / `selectedBoard` and, for folder boards, its claimed folder.
...
## Bundled board exception

Boards shipped under `assets/boards/<id>/` are app-owned registrations. They render without a
trust-file entry or trust dialog, participate in the same custom-editor and capability registries,
and appear as Built-in creatable items with a Disable action. External boards still require the
per-board trust flow described above.
