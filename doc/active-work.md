...
- **EPIC-025** — [Unified Component Library and Storybook Editor](epics/EPIC-025.md)
  ...
  - [ ] [US-542: Grid options popovers — `Popper` → UIKit `Popover` flip](tasks/US-542-grid-options-popover-flip/README.md) *(Phase 4 — overlay infrastructure cleanup; implemented — awaiting user testing + epic-close review; 2-file import swap in `editors/grid/components/{ColumnsOptions,CsvOptions}.tsx`; grep clean — zero `components/overlay/` imports remain in `src/`; tsc + lint baselines unchanged; unblocks US-532 deletion of `components/overlay/`)*
  - [ ] [US-532: Final `components/` sweep — empty the legacy folder](tasks/US-532-legacy-components-removal/README.md) *(Phase 4 — last code task before EPIC-025 close; plan ready for review; sole remaining blocker is US-542; deletes only the five primitive folders `components/{basic,form,layout,overlay,TreeView}/` plus `components/index.ts`; per user decision the four persephone-coupled folders `components/{icons,page-manager,file-search,tree-provider}/` STAY in `components/` (the folder becomes the home for persephone-coupled UIKit-consuming components); `components/{data-grid,virtualization}/` already migrated by US-536/US-538; `components/TreeView/` already callerless after US-497; `components/overlay/` becomes callerless after US-542; documentation is EXPLICITLY out of scope — all EPIC-025 doc updates are deferred to dedicated post-epic doc tasks; this task ships code-only deletions)*
