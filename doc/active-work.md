...
- **EPIC-025** — [Unified Component Library and Storybook Editor](epics/EPIC-025.md)
  ...
  - [ ] [US-536: UIKit `DataGrid` primitive — `components/data-grid/` migration](tasks/US-536-uikit-datagrid/README.md) *(Phase 4 — UIKit composite primitive; placeholder; moves AVGrid + filters + model into `uikit/DataGrid/`; 6 caller files in `editors/grid/`, `editors/graph/`, `editors/log-view/`; includes `FilterPoper` → UIKit `Popover` swap; unblocks US-532 deletion of `components/data-grid/`)*
  ...
  - [ ] [US-538: UIKit `RenderGrid` — virtualization primitive promotion](tasks/US-538-uikit-rendergrid/README.md) *(Phase 4 — UIKit foundational primitive; placeholder; relocates `components/virtualization/RenderGrid/` to `uikit/RenderGrid/`; resolves UIKit `ListBox`/`Tree` cross-folder import smell; 9 external caller files in 4 editors; unblocks US-532 deletion of `components/virtualization/`)*
