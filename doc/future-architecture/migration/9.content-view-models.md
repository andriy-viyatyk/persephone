...
- **Note:** First subtask completed early in US-057 — `NoteItemEditModel` needed `acquireViewModel`/`releaseViewModel`/`stateStorage` because embedded grid/markdown editors already use `useContentViewModel`. Remaining work: remove cast and update `AsyncEditor` typing.
