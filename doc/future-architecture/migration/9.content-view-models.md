...
export function GridEditor({ model }: { model: IContentHost }) {
    const grid = useContentViewModel<GridViewModel>(model, "grid-json");
    if (!grid) return null;  // loading (typically instant)
...
- Model comes from `host.acquireViewModel("grid-json")` via registry — validated and factory-created
...
- Works identically for standalone pages and embedded notebook notes
...
- [ ] Refactor `TextEditorModel` → `TextViewModel extends ContentViewModel`
- [ ] Register `createViewModel` factory for `"monaco"` in `register-editors.ts`
- [ ] Update `TextPageView` to use `useContentViewModel(model, "monaco")` instead of `model.editor`
- **Test:** open/edit text files, Monaco selection, content height, switch editors

### Task 3: GridViewModel
- [ ] Refactor `GridPageModel` → `GridViewModel extends ContentViewModel`
- [ ] Register `createViewModel` factory for `"grid-json"` and `"grid-csv"` in `register-editors.ts`
- [ ] Update `GridEditor` to use `useContentViewModel(model, editorId)` instead of `useComponentModel`
- **Test:** open JSON/CSV, edit cells, add/delete rows, sorting, filtering, switch grid↔Monaco

### Task 4: NotebookViewModel
- [ ] Refactor `NotebookEditorModel` → `NotebookViewModel extends ContentViewModel`
- [ ] Register `createViewModel` factory for `"notebook-view"` in `register-editors.ts`
- [ ] Update `NotebookEditor` to use `useContentViewModel(model, "notebook-view")`
- **Test:** notebook CRUD, categories, tags, search, drag-drop, expanded note view

### Task 5: TodoViewModel
- [ ] Refactor `TodoEditorModel` → `TodoViewModel extends ContentViewModel`
- [ ] Register `createViewModel` factory for `"todo-view"` in `register-editors.ts`
- [ ] Update `TodoEditor` to use `useContentViewModel(model, "todo-view")`
- **Test:** todo CRUD, lists, tags, search, drag-drop

### Task 6: MarkdownViewModel
- [ ] Refactor `MarkdownView`'s inline model → `MarkdownViewModel extends ContentViewModel`
- [ ] Register `createViewModel` factory for `"md-view"` in `register-editors.ts`
- [ ] Update `MarkdownView` to use `useContentViewModel(model, "md-view")`
- **Test:** markdown preview, search highlighting, scroll sync

### Task 7: LinkViewModel
- [ ] Refactor `LinkEditorModel` → `LinkViewModel extends ContentViewModel`
- [ ] Register `createViewModel` factory for `"link-view"` in `register-editors.ts`
- [ ] Update `LinkEditor` to use `useContentViewModel(model, "link-view")`
- **Test:** link detection, link list display

### Task 8: Pure renderers (SVG, HTML, Mermaid)
- [ ] Create thin `ContentViewModel` wrappers: `SvgViewModel`, `HtmlViewModel`, `MermaidViewModel`
- [ ] Register `createViewModel` factories for `"svg-view"`, `"html-view"`, `"mermaid-view"`
- [ ] Update view components to use `useContentViewModel(model, editorId)`
- **Test:** SVG/HTML/Mermaid preview rendering

### Task 9: NoteItemEditModel — IContentHost
- [ ] Implement `IContentHost` on `NoteItemEditModel` (acquireViewModel/releaseViewModel via ContentViewModelHost)
- [ ] Remove `as unknown as TextFileModel` cast in `NoteItemActiveEditor`
- [ ] Update `AsyncEditor` to accept `IContentHost` instead of `PageModel`
